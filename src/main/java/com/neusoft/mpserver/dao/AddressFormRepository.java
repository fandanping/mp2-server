...
public interface AddressFormRepository  extends JpaRepository<AddressMarkForm,String> {
    ...
    //精准去重
    @Modifying
    @Query("update AddressMarkForm set province=?2,city=?3,area=?4,town=?5 ,markTime=?6,status=?7,markUser=?1,marked='1' where address=?8")
    public int updateSameAddress(String  userId,String province,String city,String area,String town,String date,Integer status,String address);
}
