...
public interface WordMarkRepository  extends JpaRepository<wordmark,String> {
   ...
   /* @Transactional
    ...
    //查询特征检索式
    @Query(value = "select new com.neusoft.mpserver.jszkoffline.domain.wordmark(an,citedAn,SEARCHWORDS,categoryType,SEARCHWORDS2) from wordmark where an=?1 and citedan=?2")
    public wordmark findSearchWords(String an, String citedAn);

    //保存特征检索式
    @Transactional
    @Modifying
    @Query(value = "insert into JSZK_MARK_SEARCHWORDS(an, citedAn,SEARCHWORDS,category_Type,userId,SEARCHWORDS2) values (?,?,?,?,?,?)", nativeQuery = true)
    public int saveSearchWords2(String an,String citedAn,String words,String categoryType,String userId,String words2);

    //更新特征检索式
    @Transactional
    @Modifying
    @Query(value = "update JSZK_MARK_SEARCHWORDS set SEARCHWORDS=?3, category_Type=?4 ,userId=?5,SEARCHWORDS2=?6 where an=?1 and citedan=?2", nativeQuery = true)
    public int updateSearchWords2(String an,String citedAn,String words,String categoryType,String userId,String words2);
